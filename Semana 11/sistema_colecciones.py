...
class Producto:
    ...
    # Serialización a diccionario para JSON
    def to_dict(self):
        return {
            "id": self._id,
            "nombre": self._nombre,
...
class Inventario:
    ...
    def precargar_productos_ejemplo(self):
        ...
        for prod in ejemplos:
            self.productos[prod.get_id()] = prod
        print("Inventario inicial precargado con productos de ejemplo.")

    def cargar_desde_archivo(self):
        if not os.path.exists(self.archivo):
            print(f"Archivo {self.archivo} no encontrado, se creará nuevo al guardar.")
            return
        try:
            ...
            print(f"Error: El archivo {self.archivo} no es JSON válido.")
        except PermissionError:
            print(f"Error: Sin permisos para leer {self.archivo}.")
    ...
    def guardar_en_archivo(self):
        try:
            with open(self.archivo, 'w', encoding='utf-8') as f:
                json.dump([p.to_dict() for p in self.productos.values()], f, ensure_ascii=False, indent=4)
            print(f"Inventario guardado exitosamente en {self.archivo}.")
        except PermissionError:
            print(f"Error: Sin permisos para escribir en {self.archivo}.")
        except Exception as e:
    ...
    def buscar_productos_por_nombre(self, nombre):
        ...
        resultados = [p for p in self.productos.values() if nombre_lower in p.get_nombre().lower()]
        return resultados
